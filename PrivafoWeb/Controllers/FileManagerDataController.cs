...
namespace Kendo.Mvc.Examples.Controllers
{
    public class FileManagerDataController : Controller
    {
        ...
        {
                        //var dir = HostingEnvironment.WebRootPath;
            //var path = Path.Combine(dir, contentFolderRoot, "filemanager"); //folder name
            var virtualPath = Path.Combine(contentFolderRoot, "UserFiles", prettyName);
            //var path = HostingEnvironment.WebRootFileProvider.GetFileInfo(virtualPath).PhysicalPath;
            var provider = new PhysicalFileProvider(HostingEnvironment.WebRootPath);
            var contents = provider.GetDirectoryContents(string.Empty);
            var path = provider.GetFileInfo(virtualPath).PhysicalPath;
            if (!Directory.Exists(path))
            {
                ...
                foreach (var sourceFolder in foldersToCopy)
                {
                    ...
                    CopyFolder(provider.GetFileInfo(sourceFolder).PhysicalPath, path);
                }
        }
            return virtualPath;
        }
